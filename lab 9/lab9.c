...
#define DEFNUM_ENEMIES 5
// static const int DEFNUM_ENEMIES = DEF;

STyp Enemy[DEFNUM_ENEMIES];
STyp Player;
STyp Missile;
...
uint8_t enemyAlive[DEFNUM_ENEMIES];
uint8_t enemiesAlive;
uint8_t missleReady;
uint16_t frameDelay = 250;

uint16_t enemies_y = 10;
uint16_t score;


volatile uint8_t all_buttons  = 0xFF;
// volatile uint8_t all_buttons_prev  = 0xFF;
...
void initGame() {
	enemies_y = 10;
	for (uint8_t i = 0; i < DEFNUM_ENEMIES; i ++) {
		Enemy[i].x = i * 16;
		Enemy[i].y = enemies_y;
...
void advanceGame() {
	...
	if (Missile.y > 10) {
		...
		for (int i = 0; i < DEFNUM_ENEMIES; i++) {
			if (enemyAlive[i]) {
